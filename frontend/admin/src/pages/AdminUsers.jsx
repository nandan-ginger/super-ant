...
import { HiPlus, HiUsers, HiShieldCheck, HiPencil } from 'react-icons/hi2'
    ...
    <div className="grid grid-cols-2 gap-3 mt-2">
      {PERM_SECTIONS.map(s => (
        <div key={s.key} className="bg-slate-50 rounded-xl p-4">
          <p className="text-[10.5px] font-bold text-slate-400 uppercase tracking-wider mb-3">{s.label}</p>
          <div className="space-y-2.5">
            {s.actions.map(a => (
              <div key={a} className="flex items-center justify-between">
                <span className="text-[13px] font-medium text-slate-700 capitalize">{a}</span>
                <ToggleSwitch
                  id={`perm-${s.key}-${a}`}
...
function getPermSummary(perms, role) {
  ...
  const parts = []
  if (perms.widgets?.view)   parts.push('Widgets')
  if (perms.livechats?.view) parts.push('Chats')
  if (perms.visitors?.view)  parts.push('Visitors')
  if (perms.leads?.view)     parts.push('Leads')
